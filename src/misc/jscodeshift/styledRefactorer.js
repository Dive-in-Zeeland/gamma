...
const getStyles = source =>
  compose(stylesExpression, objectExpressionToObject)(source);

// Make camelCase string a kebab-case
const camelToKebab = camel =>
  camel.replace(/(?!^)[A-Z]/, char => `-${char.toLowerCase()}`);
...
const getCss = nativeStyle =>
  `${Object.entries(nativeStyle).reduce(
    (css, [prop, value]) =>
...
const styledDeclaration = (name, css, type) =>
  j.variableDeclaration('const', [
    j.variableDeclarator(
      j.identifier(name),
      j.taggedTemplateExpression(
        j.callExpression(j.identifier('styled'), [j.identifier(type)]),
        j.templateLiteral(
          [
            j.templateElement(
              {
...
const insertStyledComponent = (source, name, nativeStyle, type) => {
  ...
  root
    ...
    .get()
    .insertBefore(styledDeclaration(name, getCss(nativeStyle), type));
  return root.toSource();
};

// Get all elements to refactor (elements that have style = styles.something)
const getElementsWithStyle = source =>
  j(source)
    .find(j.JSXElement)
    .filter(
      nodePath =>
...
const getNameForStyledComponent = nodePath =>
  compose(
    getStyleName,
    formatName,
    removeNameCollision(getJsxTagName(nodePath)),
  )(nodePath);

// Check if styled-component with that name is already created on a source
const doesStyledComponentExist = (source, name) =>
  j(source)
    .find(j.VariableDeclarator, {
      id: {
        name,
...
const populateWithStyledComponents = source => {
  ...
  getElementsWithStyle(source).forEach(nodePath => {
    if (
      doesStyledComponentExist(newSource, getNameForStyledComponent(nodePath))
    )
      return;
...
const getAttribute = attrName => nodePath =>
  nodePath.node.openingElement.attributes.find(
...
const getStyleName = nodePath =>
  getAttribute('style')(nodePath).value.expression.property.name;

// Get tag name of a JSX element
const getJsxTagName = nodePath => nodePath.node.openingElement.name.name;
...
const removeAttrFromTag = attrName => tag => ({
  ...tag,
  attributes: tag.attributes.filter(attr => attr.name.name !== attrName),
});
...
const refactorJsx = source =>
  getElementsWithStyle(source)
    .replaceWith(nodePath =>
      compose(
        compose(getNameForStyledComponent, renameElement)(nodePath),
...
const removeStyleDeclaration = source =>
  j(source)
    .find(j.VariableDeclaration, {
      declarations: [
        {
